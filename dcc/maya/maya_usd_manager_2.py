...
class OrionUSDManager(MayaQWidgetDockableMixin, QtWidgets.QWidget):
    ...
    def refresh_context(self):
        ...
        try:
            ...
            self.ctx['end'] = int(os.environ.get("ORI_SHOT_FRAME_END", 1100)) + 20
        except:
            self.ctx['start'] = 981
            self.ctx['end'] = 1100
    ...
    def export_animation(self):
        ...
        if asset_name and "Ori" in asset_name:
            ...
            if visor_nodes:
                mesh_attr = f"{visor_nodes[0]}.visibility"
                if cmds.objExists(mesh_attr):
        ...
        warnings = []
        if wrong_context:
            warnings.append("WARNING: You are not in a valid Shot context! Are you sure you want to export here?")
        if visor_hidden:
        ...
        if warnings:
            ...
            msg += "Sanity Check Passed. Ready to export."
        ...
        if success:
            ...
            for node in all_nodes:
                ...
                if ':' in short_name:
                    #convert colon to underscore to match Maya USD formatting
                    namespace_prefix = short_name.rsplit(':', 1)[0].replace(':', '_') + '_'
                    break
    ...
    def publish_selected(self):
        ...
        msg = f"Source: {filename}\n\nPublish To:\n{clean_name}\n\n(This creates an instant, lightweight pointer file)"
        ...
        if res == "Publish":
            try:
                #try to open existing file instead of forcefully deleting it
                if os.path.exists(dst_path):
                    master_layer = Sdf.Layer.FindOrOpen(dst_path)
                    #clear any old pointers inside it
                    master_layer.subLayerPaths.clear()
                else:
                    #create new layer if it does not exist yet
                    master_layer = Sdf.Layer.CreateNew(dst_path)
                
                #calculate relative path to versioned export
                rel_path = f"../{filename}"
                
                #inject heavy versioned file
                master_layer.subLayerPaths.append(rel_path)
                
                #save back to disk
                master_layer.Save()
                
                self.lbl_pub_status.setText(f"Successfully published: {clean_name}")
                self.lbl_pub_status.setStyleSheet("color: #4CAF50;")
            except Exception as e:
                #catch file locks and alert user safely
                self.lbl_pub_status.setText("Error: File is locked by another program.")
                self.lbl_pub_status.setStyleSheet("color: #F44336;")
                
                #provide a clear popup explaining what the user needs to do
                lock_msg = f"Windows Access Denied.\n\nSomeone has {clean_name} open in Houdini or Maya.\nAll users must close this file before you can publish over it."
                cmds.confirmDialog(title="Publish Blocked", message=lock_msg, icon="critical")
